...
if [ "$startInstaller" = "n" ] || [ "$startInstaller" = "N" ]; then
	...
	read -n 1 -p "Specify keyboard layout? (uses 'us' by default) [N/y]" specifyKeyboardLayout
	if [ "$specifyKeyboardLayout" == "y" ] || [ "$specifyKeyboardLayout" == "Y" ]; then
	    echo
	    echo "Input keyboard layout country code"
