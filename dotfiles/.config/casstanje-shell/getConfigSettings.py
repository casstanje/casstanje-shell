...
def main(printOutput = False):
    ...
    with open(f"{toolsVariables.casstanjeShellDir}/config-template.json", "r") as f:
        ...
        for key,value in jsonObject.items():
            ...
            elif(key == "catppuccinaccent"):
                useDefaultValue = "catppuccinaccent" in setOptions.keys()
                if(useDefaultValue): useDefaultValue = setOptions["catppuccinaccent"] != ""
                optionsObject["catppuccinaccent"] = (
                    setOptions["catppuccinaccent"]
